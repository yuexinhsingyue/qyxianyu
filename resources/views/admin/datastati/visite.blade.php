@extends('layouts.admin')

@section('title','访问动态')

@section('content')
    <div class="col">
        <div class="card  stats" style="height: 80px;">
        <div class="stat-icon"> <i class="fa fa-rocket"></i> </div>
            <div class="stat">
                <div class="value"> &nbsp;&nbsp;&nbsp;&nbsp;{{$orderMonth}} </div>
                <div class="name" style="font-size:20px "> 网站总访问量 </div>
            </div> 
           
        </div>
    </div>
      <!-- 为ECharts准备一个具备大小（宽高）的Dom -->
    <div class="col">  
    <div class="card  stats">  
        <div id="main" style="width: 900px;height:400px;"></div>
        <script type="text/javascript" src="/admin/js/jquery.js"></script>
        <script type="text/javascript" src="/admin/js/echarts.js"></script>
        <script type="text/javascript">
            // 基于准备好的dom，初始化echarts实例
            var myChart = echarts.init(document.getElementById('main'));

            var readpoint1 = <?php echo json_encode($chart1);?>;
            var readpoint2 = <?php echo json_encode($chart2);?>;

            var echart1X = [];   //X轴数据
            var echart1Y = [];   //Y轴数据
            for(var tmp in readpoint1){

                echart1X.push(tmp);  //将键名压入X轴  日期
                echart1Y.push(readpoint1[tmp]);   //将值压入Y轴 访问量

            }
            var echart2X = [];   //X轴数据
            var echart2Y = [];   //Y轴数据
            for(var tmp in readpoint2){

                echart2X.push(tmp);  //将键名压入X轴  日期
                echart2Y.push(readpoint2[tmp]);   //将值压入Y轴 访问量
            }
            console.log(echart2X);

          app.title = '多 X 轴';

        var colors = ['#5793f3', '#d14a61', '#675bba'];
        option = {
            color: colors,
            tooltip: {
                trigger: 'none',
                axisPointer: {
                    type: 'cross'
                }
            },
            legend: {
                data:['本期 访问量', '上期 访问量']
            },
            grid: {
                top: 70,
                bottom: 50
            },
            xAxis: [
                {
                    type: 'category',
                    axisTick: {
                        alignWithLabel: true
                    },
                    axisLine: {
                        onZero: false,
                        lineStyle: {
                            color: colors[1]
                        }
                    },
                    axisPointer: {
                        label: {
                            formatter: function (params) {
                                return '访问量  ' + params.value
                                    + (params.seriesData.length ? '：' + params.seriesData[0].data : '');
                            }
                        }
                    },
//                    data: ["2016-1", "2016-2", "2016-3", "2016-4", "2016-5", "2016-6", "2016-7", "2016-8", "2016-9", "2016-10", "2016-11", "2016-12"]
                    data: echart1X
                },
                {
                    type: 'category',
                    axisTick: {
                        alignWithLabel: true
                    },
                    axisLine: {
                        onZero: false,
                        lineStyle: {
                            color: colors[0]
                        }
                    },
                    axisPointer: {
                        label: {
                            formatter: function (params) {
                                return '访问量  ' + params.value
                                    + (params.seriesData.length ? '：' + params.seriesData[0].data : '');
                            }
                        }
                    },
//                    data: ["2015-1", "2015-2", "2015-3", "2015-4", "2015-5", "2015-6", "2015-7", "2015-8", "2015-9", "2015-10", "2015-11", "2015-12"]
                    data: echart2X
                }
            ],
            yAxis: [
                {
                    type: 'value'
                }
            ],
            series: [
                {
                    name:'上期 访问量',
                    type:'line',
                    xAxisIndex: 1,
                    smooth: true,
//                    data: [2.6, 5.9, 9.0, 26.4, 28.7, 70.7, 175.6, 182.2, 48.7, 18.8, 6.0, 2.3]
                    data: echart1Y
                },
                {
                    name:'本期 访问量',
                    type:'line',
                    smooth: true,
//                    data: [3.9, 5.9, 11.1, 18.7, 48.3, 69.2, 231.6, 46.6, 55.4, 18.4, 10.3, 0.7]
                    data: echart2Y
                }
            ]
        };
            // 使用刚指定的配置项和数据显示图表。
            myChart.setOption(option);
        </script>
        </div>
    </div>
@endsection

